...
void main()
{
	...
	float t = gl_TessCoord.x;	//gl_TessCoord.x is 0 to 1, tells us how far along the line we are
	...
	//Interpolate
	vec4 interp = 0.5 * ((2 * p1) + (-p0 + p2) * t + (2 * p0 - 5 * p1 + 4 * p2 - p3) * (t * t) + (-p0 + 3 * p1 - 3 * p2 + p3) * (t * t * t));

	gl_Position = uP * interp;
